...
abstract class DaxeNode {
  ...
  void attributeDialogOnInsert([ActionFunction okfct]) {
    AttributeDialog dlg = new AttributeDialog(this, okfct);
    dlg.showOnInsert();
  }
...
//          else
   ...
   * added by Simon
   */
  void copyAttrFromXml (h.Element elem){
    
    for (DaxeAttr att in attributes) {
      if (!att.localName.startsWith("data-")){
        if(att.localName=="class"){
          elem.classes.add(att.value);        
        }else{
          elem.attributes[att.localName] = att.value;
        }
      }else if(att.localName.startsWith("data-toattr-")){
        List<String> dataString = att.localName.split('-');
        String attrname = dataString[2];
        String attrval = att.value;
        //elem.attributes[dataString[3]] = elem.attributes[dataString[3]]+ " " + att.value;
        if(attrname == "class"){
          elem.classes.add(att.value);  
        }else{
