...
class NodeFactory {
  ...
  static void addCoreDisplayTypes() {
    ...
    NodeFactory.addDisplayType('section',
        (x.Element ref) => new DNSection.fromRef(ref),
        (x.Node node, DaxeNode parent) => new DNSection.fromNode(node, parent)
    );
    NodeFactory.addDisplayType('layout',
        (x.Element ref) => new DNLayout.fromRef(ref),
        (x.Node node, DaxeNode parent) => new DNLayout.fromNode(node, parent)
    );
    NodeFactory.addDisplayType('block',
  ...
  static DaxeNode createFromNode(x.Node n, DaxeNode parent) {
    ...
    if (n is x.Element) {
      ...
      //h.window.alert(parent.ref.toString());
    } else {
      ref = null;
    }
