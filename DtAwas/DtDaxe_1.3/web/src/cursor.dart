...
class Cursor {
  ...
  //Added by Simon, fix chinese ime issue
  bool isime;
  ...
  void keyUp(h.KeyboardEvent event) {
    ...
    if (ctrl && !shift && keyCode == h.KeyCode.Z) { // Ctrl Z
      ...
      if(!isime || keyCode == 32 || (keyCode >= 48 && keyCode <= 57)){
        String v = ta.value;
        ta.value = '';
        doc.insertNewString(v, shift);
      }
  ...
  bool pasteString(String s){
    ...
    try{
      ...
      return(true);
        ...
        h.window.alert("something goes wrong :(");
        return(false);
      }
    return(true);
