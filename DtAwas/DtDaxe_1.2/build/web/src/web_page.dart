...
class WebPage {
  ...
  void openDocumentFromPost(String configPath) {


    doc.openDocumentFromPost(configPath).then( (_) {
      _buildMenus();
      init();
      h.document.title = "x";
    }, onError: (DaxeException ex) {
  ...
  void _buildMenus() {
    ...
    MenuItem item;
    if (doc.saveURL != null) {
      item = new MenuItem(Strings.get('menu.save'), () => save());
      fileMenu.add(item);
    }
    if(doc.textAreaId != null){
      item = new MenuItem(Strings.get('menu.finish'), () => sendAndClose(), shortcut: 'Q');
            fileMenu.add(item);
