...
class DaxeDocument {
   ...
   */
  Future newDocument(String configPath, [String rootTag]) {
    Completer completer = new Completer();
    cfg = new Config();
    cfg.load(configPath, schemaPath).then((_) {
      filePath = null;
  ...
  /**
   * Open the document at postMessage
   */
  Future openDocumentFromPost(String configPath) {
    Completer completer = new Completer();
    cfg = new Config();
    cfg.load(configPath, schemaPath).then((_) {
      getXmlFromPost().then((xmlString){
        if (xmlString.trim() != "" && xmlString != null){
        x.DOMParser dp = new x.DOMParser();
        dndoc = NodeFactory.createFromNode(dp.parseFromString(xmlString), null);
        completer.complete();
        }else {
          dndoc = new DNDocument();
          List<x.Element> roots = cfg.rootElements();   
          var rootPos = 0;
          if (rootNum != null){
            rootPos = int.parse(rootNum);
            }
          //if (roots.length == 1) {
            DaxeNode root = NodeFactory.create(roots[rootPos]);
            cfg.addNamespaceAttributes(root);
            //Position pos = page.getSelectionStart();
            if (cfg.elementAttributes(roots[0]).length > 0)
              root.attributeDialogOnInsert();
            dndoc.appendChild(root);
            root.updateValidity();
          //}
          completer.complete();
  ...
  /**
   * get a xml string from postMessage, added by Simon
   */
  Future getXmlFromPost() {
    Completer completer = new Completer();
    var xmlString = null;
    var listenMessage;
    listenMessage = h.window.onMessage.listen((event){
      xmlString = event.data;
      //h.window.alert("get!");
      listenMessage.cancel();
      completer.complete(xmlString);
    });
    h.window.parent.postMessage("get", h.window.location.href);
    return(completer.future);
  }
  
  
  /**
   * Send the document with a postMessage. added by Simon
   */
  void saveOnPostMessage() {
    h.window.parent.postMessage(toString(), h.window.location.href);
  ...
  void insertNewNode(x.Element ref, String nodeType/*, [bool checkValidity=false]*/) {
    ...
    */
    ...
    if (nodeType == 'element' && doc.cfg.elementAttributes(ref).length > 0)
      dn.attributeDialogOnInsert(() => insert2(dn, pos));
    else
      insert2(dn, pos);
  }
